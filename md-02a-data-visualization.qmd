...
link: <https://www.perplexity.ai/search/2bb0a257-107a-425a-bf12-a8361268db65?s=u>
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: The geom_bar() function expects pre-summarized data, and you need to specify the y-values explicitly. On the other hand, geom_col() is often used with raw data, and it automatically counts the number of observations in each group to determine the heights of the bars. Both functions create bar plots, but they are suited to different types of data.
...
6.  Add a new code chunk below.

7.  Use the `ggplot()` function and the `gapminder_2007` object to create a histogram using the `geom_histogram()` function with the following aesthetic mappings:
